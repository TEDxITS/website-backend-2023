...
export const register = async (
	...
	const {
		...
		error: Joi.ValidationError | undefined
		value: RegisterRequest | undefined
	} = registerSchema.validate(req.body, { abortEarly: false })

	if (error) {
		sendError(
			res,
			new CustomError(StatusCodes.BAD_REQUEST, "Invalid input"),
...
export const verifyAccount = async (
	...
	if (!verifyToken) {
		sendError(
			res,
			new CustomError(StatusCodes.BAD_REQUEST, "No token found"),
			["token"]
		)
		return
	}
...
export const refreshAdminToken = (req: Request, res: Response) => {
	...
	jwt.verify(token, env.ADMIN_REFRESH_TOKEN_KEY, async (err, decoded) => {
		...
		try {
			refreshAdminToken
			const data = await AdminAuthService.refreshAdminToken(
				decoded as AdminTokenData
			)
...
export const refreshUserToken = (req: Request, res: Response) => {
	...
	jwt.verify(token, env.USER_REFRESH_TOKEN_KEY, async (err, decoded) => {
		...
		try {
			const data = await UserAuthService.refreshUserToken(
				decoded as UserTokenData
			)
