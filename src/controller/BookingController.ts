...
export const getBookingDetailById = async (req: Request, res: Response) => {
	...
	try {
		const booking = await BookingService.getBookingDetailById(
			bookingId as string
		)
...
export const getBookingDetailsByBookingId = async (
	...
	try {
		const booking = await BookingService.getBookingDetailsByBookingId(
			userId,
			isAdmin,
			bookingId as string
		)
...
export const createBooking = async (req: Request, res: Response) => {
	...
	const {
		...
		value: Array<BookingDetailRequest> | undefined
	} = createBookingSchema.validate(data)
	if (error) {
		sendError(res, new CustomError(StatusCodes.BAD_REQUEST, error.message))
		return
	}
	...
	try {
		const booking = await BookingService.createBooking(
			userId,
			bookingData as Array<BookingDetailRequest>
		)
...
export const uploadPaymentProof = async (req: Request, res: Response) => {
	...
	const {
		error,
		value: uploadPaymentData,
	}: {
		error: Joi.ValidationError | undefined
		value: UploadPaymentRequest | undefined
	} = uploadPaymentSchema.validate(data)
	if (error) {
		sendError(res, new CustomError(StatusCodes.BAD_REQUEST, error.message))
		return
	}
	...
	try {
		const booking = await BookingService.uploadPaymentProof(
			userId,
			uploadPaymentData as UploadPaymentRequest
		)
...
export const verifyBooking = async (req: Request, res: Response) => {
	...
	if (!bookingId) {
		sendError(
			res,
			new CustomError(StatusCodes.BAD_REQUEST, "Id is required")
		)
		return
	}
