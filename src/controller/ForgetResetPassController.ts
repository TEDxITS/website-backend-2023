...
} from "../model/ForgetResetPassModel"
import { CustomError } from "../helper/Error"
import {
	resetPassword,
	sendForgetPasswordEmail,
} from "../service/ForgetResetPassService"
...
export const forgetPassword = async (req: Request, res: Response) => {
	const {
		...
		value: ForgetPasswordRequest | undefined
	} = forgetPasswordSchema.validate(req.body, { abortEarly: false })
...
interface ResetPassTokenParams {
	token: string
}

export const resetUserPassword = async (
	req: Request<ResetPassTokenParams>,
	res: Response
) => {
	const {
		error,
		...
		error: Joi.ValidationError | undefined
		value: ResetPasswordRequest | undefined
	} = resetPasswordSchema.validate(req.body, { abortEarly: false })
	...
	if (!resetPassToken) {
		sendError(
			res,
			new CustomError(StatusCodes.BAD_REQUEST, "Token is Required")
		)
		return
	}
